                            ...
                            for (int otherPriority = thisPriority; otherPriority >= lowerPriorityBound; otherPriority--)
                            {
                                ...
                                for (int candidateIndex = currentIncomingIndex; candidateIndex < blockCount; candidateIndex++)
                                {
                                    ...
                                    if (incomingBuilding + outCandidateBuilding != 0)
                                    {
                                        ...
                                        {
                                            TransferLogging.AddEntry(material, true, thisPriority, otherPriority, incomingBuilding, outCandidateBuilding, TransferLogging.MatchStatus.PathFailure, incomingOfferToMatch.Exclude, outgoingOfferCandidate.Exclude, incomingOfferToMatch.Position, outgoingOfferCandidate.Position);
                                            continue;
                                        }
                                        ...
                                        else if (candidateAI is WarehouseAI outgoingWarehouseAI)
                                        {
                                            ...
                                            {
                                                TransferLogging.AddEntry(material, true, thisPriority, otherPriority, incomingBuilding, outCandidateBuilding, TransferLogging.MatchStatus.NoVehicle, incomingOfferToMatch.Exclude, outgoingOfferCandidate.Exclude, incomingOfferToMatch.Position, outgoingOfferCandidate.Position);
                                                continue;
                                            }
                                        ...
                                        if (!Matching.ChecksPassed(incomingBuilding, outCandidateBuilding, incomingDistrict, districtManager.GetDistrict(outCandidatePosition), incomingPark, districtManager.GetPark(outCandidatePosition), material, out TransferLogging.MatchStatus result))
                                        {
                        ...
                        for (int otherPriority = thisPriority; otherPriority >= lowerPriorityBound; otherPriority--)
                        {
                            ...
                            for (int candidateIndex = currentOutgoingIndex; candidateIndex < blockCount; candidateIndex++)
                            {
                                ...
                                if (outgoingBuilding + inCandidateBuilding != 0)
                                {
                                    ...
                                    {
                                        TransferLogging.AddEntry(material, false, otherPriority, thisPriority, inCandidateBuilding, outgoingBuilding, TransferLogging.MatchStatus.PathFailure, incomingOfferCandidate.Exclude, outgoingOfferToMatch.Exclude, incomingOfferCandidate.Position, outgoingOfferToMatch.Position);
                                        continue;
                                    }
                                    ...
                                    if (outgoingAI is WarehouseAI outgoingWarehouseAI)
                                    {
                                        ...
                                        {
                                            TransferLogging.AddEntry(material, false, otherPriority, thisPriority, inCandidateBuilding, outgoingBuilding, TransferLogging.MatchStatus.NoVehicle, incomingOfferCandidate.Exclude, outgoingOfferToMatch.Exclude, incomingOfferCandidate.Position, outgoingOfferToMatch.Position);
                                            continue;
                                        }
                                    ...
                                    if (!Matching.ChecksPassed(inCandidateBuilding, outgoingBuilding, districtManager.GetDistrict(inCandidatePosition), outgoingDistrict, districtManager.GetPark(inCandidatePosition), outgoingPark, material, out TransferLogging.MatchStatus result))
                                    {
